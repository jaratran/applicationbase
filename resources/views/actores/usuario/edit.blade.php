...
<div class="container app-page">
    <div class="app-page-header">
        <div>
            ...
            <p class="app-page-description">Actualice los datos personales y organizacionales de la cuenta.</p>
        </div>
    </div>
    <div class="row">
        <div class="col-12">
            <div class="card">
                <form id="form_editar_usuario" action="{{ route('usuario.update', $usuario->id) }}" method="POST" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    {{ method_field('PUT') }}
                    ...
                    <div class="card-body">
                        ...
                        <div class="bg-light border rounded p-3 mb-4">
                            ...
                            <div class="row">
                                <div class="form-group col-md-4">
                                    ...
                                    <input type="text" class="form-control rut" name="rut_usuario" id="rut_usuario" value="{{ $usuario->rut_usuario }}" placeholder="Ej: 12.345.678-9" required maxlength="12" autofocus>
                                </div>
                                <div class="form-group col-md-4">
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <input type="text" class="form-control" name="apellidos_usuario" id="apellidos_usuario" value="{{ $usuario->apellidos_usuario }}" required>
                                </div>
                            </div>
								...
								<div class="col-md-8">
									<div class="row">
										...
										<div class="form-group col-md-6 mt-4">
											...
											<input type="text" class="form-control" name="telefono" id="telefono" value="{{ $usuario->telefono }}" required placeholder="+ 569 11 111 111">
										</div>
									</div>
									...
									<div class="row">
										...
										<div class="form-group col-md-6 mt-4 pt-3 d-none" id="div_asignar_liberar" name="div_asignar_liberar">
									...
									<div class="row">
										<div class="form-group col-md-6 mt-4">
											<label for="region_operativa">Región Operativa</label>
											<input type="text" class="form-control" id="region_operativa_codigo" value="{{ $usuario->region_operativa_codigo }}" disabled>
										</div>

										<div class="form-group col-md-6 mt-4 d-none" id="div_sucursal_usuario">
											...
											<input type="hidden" id="sucursal_actual" value="{{ $usuario->sucursal_id }}">
										</div>
										<div class="form-group col-md-6 mt-4 d-none" id="div_empresa_usuario">
											...
											<input type="hidden" id="empresa_actual" value="{{ $usuario->empresa_id }}">
										</div>
									</div>
                                </div>

								<div class="col-md-4 d-flex flex-column align-items-center justify-content-center mt-3">
									<img src="{{ asset('uploads/avatar/' . ($usuario->avatar ? $usuario->avatar . '_medium.jpg' : 'default_medium.jpg')) }}"
										alt="Avatar Mediano"
										class="img-thumbnail"
										style="max-width: 150px; height: auto; display: block; margin: auto;">
									<label>Avatar</label>
						...
						<div class="bg-light border rounded p-3">
                            ...
                            <div class="row">
                                <div class="form-group col-md-4">
                                    <label>Región</label>
                                    <select class="form-control select2" id="region_usuario" required>
                                        <option value="">{{ $usuario->comuna->region->nombre }}</option>
                                    </select>
                                    <input type="hidden" id="region_actual" value="{{ $usuario->comuna->region_id }}">
                                </div>
                                <div class="form-group col-md-4">
                                    <label>Comuna</label>
                                    <select class="form-control select2" id="comuna_usuario" name="comuna" required>
                                        <option value="">{{ $usuario->comuna->nombre }}</option>
                                    </select>
                                    <input type="hidden" id="comuna_seleccionada" value="{{ $usuario->comuna_id }}">
                                </div>
                                <div class="form-group col-md-4">
                                    <label>Dirección</label>
                                    <input class="form-control" name="direccion" value="{{ $usuario->direccion }}" required>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-footer">
                        <button type="submit" class="btn btn-primary my-2"><i class="fa fa-edit"></i> Actualizar Usuario</button>
    ...
    <script>
		function empresaUsuarioRol(rolId) {
			if (rolId == ROL_SOLICITANTE_PRODUCTOR) {
				$.get("/empresas/tipo/" + TIPO_EMPRESA_PRODUCTORA + "/por-rol", {
						...
						// console.log('Hola, Empresa Asignada : ', getEmp);
						for (var i = 0; i < data.length; i++) {
							// console.log('Id Empresa Selector : ', data[i]['id']);
		...
		function sucursalUsuarioRol(rolId) {
			if ((rolId == ROL_SOLICITANTE_PLANTA) || (rolId == ROL_SOLICITANTE_PLANTA_XII)) {
				$.get("/sucursales/tipo/" + TIPO_SUCURSAL_PLANTA + "/por-rol", {
						...
						// console.log('Hola, Sucursal Asignada : ', getSuc);
						for (var i = 0; i < data.length; i++) {
							// console.log('Id Sucursal Selector : ', data[i]['id']);
		...
		$(document).ready(function () {
            ...
            $('#form_editar_usuario').on('submit', function(e) {
                ...
                const $btnSubmit = $('#form_editar_usuario button[type="submit"]');
                if ($btnSubmit.length) {
            ...
            // Selector que permite inicializar elementos según el rol actual del usuario
            switch( parseInt($('#rol_actual').val()) ){
                case ROL_SOLICITANTE_PLANTA:
                case ROL_SOLICITANTE_PLANTA_XII:
                    if ( $selectSucursalUsuario.val() !== "" ) {
                        $btnAsignarLiberar.text('Cancelar Asignación de Planta');
