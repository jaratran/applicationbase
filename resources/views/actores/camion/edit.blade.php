    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <form action="{{ route('camion.update', $camion->id) }}" method="POST" id="formEdit">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        ...
                        <div class="bg-light border rounded p-3 mb-4">
                            ...
                            <div class="row">
                                <div class="form-group col-md-4">
                                    ...
                                    <input type="text" class="form-control" name="patente" id="patente" value="{{ $camion->patente }}" required>
                                </div>
                                <div class="form-group col-md-4">
                                    <label for="empresa_id">Empresa</label>
                                    <select class="form-control select2" id="empresa_id" name="empresa_id" required>
                                        <option value="">{{ $camion->empresa->razon_social ?? '-' }}</option>
                                    </select>
                            ...
                            <div class="row mt-3">
								<div class="form-group col-md-4">
									...
									<input type="hidden" id="region_operativa_actual" value="{{ $camion->region_operativa_id }}">
								</div>
                                <div class="form-group col-md-4">
                                    <label for="tipo_camion_id">Tipo de Camión</label>
                                    <select class="form-control select2" id="tipo_camion_id" name="tipo_camion_id" required>
                                        <option value="">{{ $camion->tipoCamion->Nombre ?? '-' }}</option>
                                    </select>
                                    <input type="hidden" id="tipo_actual" value="{{ $camion->tipo_camion_id }}">
                                </div>
                                <div class="form-group col-md-4">
                                    <label for="rendimiento_optimo">Rendimiento Óptimo</label>
                                    <input type="number" step="0.01" class="form-control" name="rendimiento_optimo" id="rendimiento_optimo" value="{{ $camion->rendimiento_optimo }}">
                                </div>
                            </div>
                            ...
                            <div class="row mt-3">
                                <div class="form-group col-md-4">
                                    ...
                                    <select class="form-control" name="arrendado" id="arrendado">
                                        <option value="0" {{ !$camion->arrendado ? 'selected' : '' }}>No</option>
                                        <option value="1" {{ $camion->arrendado ? 'selected' : '' }}>Sí</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-light border rounded p-3">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <select class="form-control select2" id="conductor_id" name="conductor_id" required>
                                    <option value="{{ $camion->conductor_id }}" selected>
                                        {{ $camion->conductor->nombre ?? '-' }} {{ $camion->conductor->apellido ?? '' }}
                                    </option>
                                </select>

                                <input type="hidden" id="conductor_actual" value="{{ $camion->conductor_id }}">
                            </div>
                        </div>
                    </div>

                    <div class="card-footer">
                        <button type="submit" class="btn btn-primary my-2"><i class="fa fa-edit"></i> Actualizar Camión</button>
...
@section('endbody-scripts')
    ...
    // Carga de Selectores de Empresa Transportista y Conductor por Defecto
    <script>
        $(document).ready(function () {
            function cargarConductorDefecto(empresaID) {
                $.get("/conductores/empresa/" + empresaID, function(data) {
                    var actual = $("#conductor_actual").val();
                    let html = '<option value="">Seleccione Conductor</option>';
                    data.forEach(s => {
                        html += `<option value="${s.id}"${s.id == actual ? ' selected' : ''}>${s.nombre} ${s.apellido}</option>`;
                    });
			...
			function transportistaCamion() {
                $.get("/empresas/tipo/" + TIPO_EMPRESA_TRANSPORTISTA ,function(data) {  // tipo_empresa = 33 -> Id de Transportista en Catalogos
                    ...
                    // Cargar conductores al inicio (teniendo seleccionada la empresa transportista)
                    if (getEmpresa && getEmpresa !== "") {
                        cargarConductorDefecto(getEmpresa);
                    }
            ...
            transportistaCamion();
        	catalogo2select2(CATEGORIA_TIPO_CAMION, "tipo_camion_id", "Seleccione Tipo de Camion", "tipo_actual");		// Edit si considera valor pre-cargado
        });
    </script>
