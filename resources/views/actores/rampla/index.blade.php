            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group col-md-12 mb-2">
                        <button type="button" class="btn btn-primary mb-2" onclick="window.location.href='{{ route('rampla.create') }}'">
                            <i class="fa fa-plus"></i> Nueva Rampla
                        </button>
                    </div>

                    <div id="ramplasTable" class="datatable-contenedor-externo">
                            ...
                            <div class="col-12 col-md-8">
								...
								<div id="Tipo" class="filtro d-flex flex-wrap align-items-center gap-2 w-75">
									...
									<div class="filtros-grid mt-2" role="group" aria-label="Filtro Tipo">
										<div class="col-4 col-sm-auto">
											<input type="checkbox" class="btn-check filtro-toggle" id="tipo1" value="Rifer" autocomplete="off" checked>
											<label class="btn filtro-btn-secondary w-100" for="tipo1">Rifer</label>
										</div>
										<div class="col-4 col-sm-auto">
											<input type="checkbox" class="btn-check filtro-toggle" id="tipo2" value="Rampla" autocomplete="off" checked>
											<label class="btn filtro-btn-secondary w-100" for="tipo2">Rampla</label>
										</div>
									</div>
								</div>
								...
								<tbody>
									@foreach ($ramplas as $rampla)
										<tr>
											...
											<td>{{ $rampla->patente }}</td>
											<td class="text-center">{{ $rampla->region_operativa_codigo }}</td>
											<td>{{ $rampla->tipoRampla->nombre ?? '-' }}</td>
											<td>{{ $rampla->capacidadRampla->nombre ?? '-' }}</td>
											<td>{{ $rampla->estadoRampla->nombre ?? '-' }}</td>

											<td>
												<div class="d-grid gap-1">
													<div class="d-flex justify-content-center gap-1">
														<button type="button"
																class="btn btn-info btn-xs text-white btnShowRampla"
																data-id="{{ $rampla->id }}">
															<i class="fa fa-eye"></i>
														</button>

                                                        @if ($rampla->activo)
                                                            <a class="btn btn-warning btn-xs text-white" href="{{ route('rampla.edit', ['rampla' => Crypt::encrypt($rampla->id)]) }}">
                                                    ...
                                                    <div class="d-flex justify-content-center gap-1">
                                                        @if ($rampla->activo)
                                                            <button type="button" class="btn btn-danger btn-xs btnDeleteRampla" data-id="{{ $rampla->id }}" data-activo="{{ $rampla->activo ? '1' : '0' }}">
...
<!-- Modal: Detalle -->
<div class="modal fade" id="modalRampla" tabindex="-1" aria-labelledby="modalRamplaLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="modalRamplaLabel">Detalle de Rampla</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Cerrar"></button>
            </div>
            <div class="modal-body" id="detailShowRampla"></div>
            <div class="modal-footer">
                <a type="button" id="btnPrintRampla" class="btn btn-primary" target="_blank"><i class="fa fa-print"></i> Imprimir</a>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-warning text-dark">
                <h5 class="modal-title" id="registroInactivoLabel">Rampla Inactiva</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
            </div>

            <div class="modal-body text-center">
                <i class="fas fa-ban text-warning mb-3" style="font-size: 4rem;"></i>
                <p class="fs-5">Rampla inactiva no puede ser editada.</p>
...
@section('endbody-scripts')
    ...
    <script>
        $(document).ready(function () {
            inicializarDataTable('ramplasTable');
        });

        $(window).on('resize', function () {
            actualizarScrollHorizontal('ramplasTable');
        });
    </script>

    <script src="https://cdn.jsdelivr.net/npm/handlebars@latest/dist/handlebars.min.js"></script>
    @include('actores.rampla.template-handlebars-show')

	<script>
        const modalRampla = new bootstrap.Modal(document.getElementById('modalRampla'));

        $(document).on("click", ".btnShowRampla", function () {
            const id = $(this).data("id");

			$.get(`/actores/rampla/${id}`, function (data) {
                const source = document.getElementById("template-show-rampla").innerHTML;
                const template = Handlebars.compile(source);
                const html = template(data);
                $("#detailShowRampla").html(html);
                modalRampla.show();
            });
        });

        document.getElementById("btnPrintRampla").addEventListener("click", function (e) {
            e.preventDefault();
            const emblemaURL = "{{ url('/config/'.$designParameter['emblema_design']) }}";
            const contenido = document.getElementById("detailShowRampla").innerHTML;
            const ventana = window.open("", "_blank", "width=800,height=600");

            ventana.document.write(`
                <html>
                    <head>
                        <title>Imprimir Detalle de la Rampla</title>
                    ...
                    <body>
                        ...
                        <h3 class="mb-4 text-center">Detalle de la Rampla</h3>
   ...
   <!-- Procesamiento de la ventana modal de Anulación -->
    <script>
        ...
        $(document).on("click", ".btnDeleteRampla", function () {
            // Asignación de la acción al Form
            const id = this.dataset.id;
            const deleteForm = document.getElementById('deleteForm');
            deleteForm.action = `/actores/rampla/${id}`;
            ...
            pregunta.innerText = `¿Está seguro que desea ${activo ? 'desactivar' : 'reactivar'} esta rampla?`;
